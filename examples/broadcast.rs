    ...
    run(async {
        ...
        builder.spawn_machine(
            ...
            |_: mpsc::Receiver<()>, mut ev: mpsc::Sender<()>| async move {
                let socket = async_io::Async::<UdpSocket>::bind((Ipv4Addr::UNSPECIFIED, 5353)).unwrap();
                let multicast = [224, 0, 0, 251].into();
                socket.get_ref().join_multicast_v4(&multicast, &Ipv4Addr::UNSPECIFIED).unwrap();
                loop {
                    let mut buf = [0u8; 11];
        ...
        builder.spawn_machine(
            ...
            move |mut cmd: mpsc::Receiver<()>, _: mpsc::Sender<()>| async move {
                let socket = async_io::Async::<UdpSocket>::bind((Ipv4Addr::UNSPECIFIED, 0)).unwrap();
                let multicast = [224, 0, 0, 251].into();
                socket
